...
function mockConfig(options) {
    ...
    var hasInclude = options.include;  //新增多swagger的path合并到mock.config.js
    var mockFilePath = options.file;
    var port = options.server;
            ...
            inquirer.prompt(createConfigQuestion).then(function(answers) {
                if (answers.create) {
                    ...
                    fs.writeFile(mock.configFilePath, '', (err) => {
                        ...
                        if (hasInclude) {
                            // ===多swagger文件合并到config===
                            fileMergeConfig(mock.swaggerFilePath); //多swagger文件合并到config
                        }
                    });
                } else {
                    // 不同意自动生成，需先遍历查找
...
//===多swagger文件path合并到mock.config.js===
function fileMergeConfig (obj) {
    console.log(obj);
}
